...
x_cols_drop = ["depth", "full_reads_percent", "publish_date", "session", "document_id", 'date', 'title', 'text']
...
search = loads(DATA_PATH / "depth_0reg.pickle")
logger.log(msg="depth", level=logging.getLevelName("WARNING"))
score = calculate_score(y, search.predict(X), ['depth'])
