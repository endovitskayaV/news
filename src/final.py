...
categs = {
    '1': (datetime.strptime('19-02-2000', '%d-%m-%Y').date(), datetime.strptime('19-03-2022', '%d-%m-%Y').date()),
    '3': (datetime.strptime('20-03-2022', '%d-%m-%Y').date(), datetime.strptime('08-04-2022', '%d-%m-%Y').date()),
    '4': (datetime.strptime('09-04-2022', '%d-%m-%Y').date(), datetime.strptime('19-02-2025', '%d-%m-%Y').date()),
}
df_dict = {category_name: df_train[(df_train['date'] >= start_date) & (df_train['date'] <= end_date)] for
           category_name, (start_date, end_date) in
           categs.items()}
...
for category, df in df_dict.items():
    ...
    cats_df_dict = {category_name: df[df['category'].isin(category_ids)] for category_name, category_ids in
               cats.items()}

    for category, df_train in cats_df_dict.items():
        logger.log(msg="category " + category, level=logging.getLevelName("WARNING"))
