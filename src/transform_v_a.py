...
def encode_list_by_rate(df: DataFrame, col_name: str, rate_limit: float) -> DataFrame:
    ...
    dump(DATA_PATH / "col_encode_dict.pickle", col_encode_dict)
...
# transform data
#
# df_train1 = pd.read_csv(DATA_PATH / "df_text.csv", parse_dates=['publish_date'])
# #
# df_train1.sort_values('publish_date', inplace=True)
# df_train1['Time'] = np.arange(len(df_train1.index))
...
#
# df_train1 = df_train1[df_train1.category.isin(
...
# #
# df_train1 = df_train1[df_train1['views'] <= 800_000]
# df_train1 = df_train1[df_train1['depth'] < 1.79]
# df_train1.loc[df_train1['full_reads_percent'] > 100, 'full_reads_percent'] = np.nan
# df_train1['full_reads_percent'].fillna((df_train1['full_reads_percent'].mean()), inplace=True)
# #
# df_train1 = encode_list_by_rate(df_train1, 'authors', 0.03)
#
...
# df_train.to_csv(DATA_PATH / "df_text_prepared_a_short.csv", index=False)
...
x_cols_drop = ["full_reads_percent", "publish_date", "session", "document_id", 'date', 'title',
               'text']
...
def train_score(index: int, y_cols: List[str], X_train, X_test, y_train, y_test):
    ...
    for n_components_rate in [0]:
        ...
        dump(DATA_PATH / (str(index) + "reg_authors_short_full_rp.pickle"), search)
